...
E-mail subjects, JIRA issue summaries, PagerDuty alerts, Squadcast alerts, or any alerter that has a "subject" can be customized by adding an ``alert_subject``
...
Squadcast alerter will trigger an incident to a predefined Squadcast webhook url.

The alerter requires the following options:

``squadcast_webhook_url``: URL generated by Squadcast for the Elastalert service.
