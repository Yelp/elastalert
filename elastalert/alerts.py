...
class StompAlerter(Alerter):
    ...
    def alert(self, matches):
        ...
        conn = stomp.Connection(
            [(self.stomp_hostname, self.stomp_hostport)], use_ssl=self.stomp_ssl
        )
        ...
        if self.bump_tickets:
            ...
            if ticket:
                ...
                if ts_to_dt(ticket.fields.updated) >= inactivity_datetime:
                    ...
                    return None
                elastalert_logger.info(
                    'Commenting on existing ticket %s' % (ticket.key)
                )
                for match in matches:
                    try:
                ...
                if self.transition:
                    elastalert_logger.info(
                        'Transitioning existing ticket %s' % (ticket.key)
                    )
                    try:
...
class HipChatAlerter(Alerter):
    ...
    def __init__(self, rule):
        ...
        self.hipchat_ignore_ssl_errors = self.rule.get(
            'hipchat_ignore_ssl_errors', False
        )
    ...
    def get_info(self):
        return {'type': 'hipchat', 'hipchat_room_id': self.hipchat_room_id}
...
class MsTeamsAlerter(Alerter):
    ...
    def __init__(self, rule):
        ...
        self.ms_teams_alert_fixed_width = self.rule.get(
    ...
    def get_info(self):
        return {'type': 'ms_teams', 'ms_teams_webhook_url': self.ms_teams_webhook_url}
...
class MattermostAlerter(Alerter):
    ...
    def __init__(self, rule):
        ...
        self.mattermost_ignore_ssl_errors = self.rule.get(
            'mattermost_ignore_ssl_errors', False
        )

        # Override webhook config
        self.mattermost_username_override = self.rule.get(
            'mattermost_username_override', 'elastalert'
        )
        self.mattermost_channel_override = self.rule.get(
            'mattermost_channel_override', ''
        )
        self.mattermost_icon_url_override = self.rule.get(
    ...
    def get_aggregation_summary_text__maximum_width(self):
        width = super(
            MattermostAlerter, self
        ).get_aggregation_summary_text__maximum_width()
    ...
    def populate_fields(self, matches):
        ...
        for field in self.mattermost_msg_fields:
            ...
            if 'args' in field:
                args_values = [
                    lookup_es_key(matches[0], arg) or missing for arg in field['args']
                ]
                if 'value' in field:
...
class PagerDutyAlerter(Alerter):
    ...
    def __init__(self, rule):
        ...
        self.pagerduty_incident_key_args = self.rule.get(
            'pagerduty_incident_key_args', None
        )
        ...
        self.pagerduty_v2_payload_class = self.rule.get(
            'pagerduty_v2_payload_class', ''
        )
        self.pagerduty_v2_payload_class_args = self.rule.get(
            'pagerduty_v2_payload_class_args', None
        )
        self.pagerduty_v2_payload_component = self.rule.get(
            'pagerduty_v2_payload_component', ''
        )
        self.pagerduty_v2_payload_component_args = self.rule.get(
            'pagerduty_v2_payload_component_args', None
        )
        self.pagerduty_v2_payload_group = self.rule.get(
            'pagerduty_v2_payload_group', ''
        )
        self.pagerduty_v2_payload_group_args = self.rule.get(
            'pagerduty_v2_payload_group_args', None
        )
        self.pagerduty_v2_payload_severity = self.rule.get(
            'pagerduty_v2_payload_severity', 'critical'
        )
        self.pagerduty_v2_payload_source = self.rule.get(
            'pagerduty_v2_payload_source', 'ElastAlert'
        )
        self.pagerduty_v2_payload_source_args = self.rule.get(
            'pagerduty_v2_payload_source_args', None
        )
    ...
    def alert(self, matches):
        ...
        if self.pagerduty_api_version == 'v2':
            ...
            match_timestamp = lookup_es_key(
                matches[0], self.rule.get('timestamp_field', '@timestamp')
            )
            if match_timestamp:
    ...
    def get_incident_key(self, matches):
        if self.pagerduty_incident_key_args:
            incident_key_values = [
                ...
                for arg in self.pagerduty_incident_key_args
            ]
            ...
            missing = self.rule.get('alert_missing_value', '<MISSING VALUE>')
            incident_key_values = [
                missing if val is None else val for val in incident_key_values
            ]
...
class PagerTreeAlerter(Alerter):
    ...
    def get_info(self):
        return {'type': 'pagertree', 'pagertree_integration_url': self.url}
...
class TwilioAlerter(Alerter):
    ...
    def get_info(self):
        return {'type': 'twilio', 'twilio_client_name': self.twilio_from_number}
...
class VictorOpsAlerter(Alerter):
    ...
    required_options = frozenset(
        ['victorops_api_key', 'victorops_routing_key', 'victorops_message_type']
    )
    ...
    def __init__(self, rule):
        ...
        self.victorops_entity_display_name = self.rule.get(
            'victorops_entity_display_name', 'no entity display name'
        )
        self.url = (
            'https://alert.victorops.com/integrations/generic/20131114/alert/%s/%s'
...
class TelegramAlerter(Alerter):
    ...
    def alert(self, matches):
        ...
        auth = (
            HTTPProxyAuth(self.telegram_proxy_login, self.telegram_proxy_password)
        ...
        try:
            ...
            raise EAException(
                "Error posting to Telegram: %s. Details: %s"
                % (e, "" if e.response is None else e.response.text)
            )

        elastalert_logger.info("Alert sent to Telegram room %s" % self.telegram_room_id)

    def get_info(self):
        return {'type': 'telegram', 'telegram_room_id': self.telegram_room_id}
...
class GoogleChatAlerter(Alerter):
    ...
    def __init__(self, rule):
        ...
        self.googlechat_header_subtitle = self.rule.get(
            'googlechat_header_subtitle', None
        )
        self.googlechat_header_image = self.rule.get('googlechat_header_image', None)
        self.googlechat_footer_kibanalink = self.rule.get(
            'googlechat_footer_kibanalink', None
        )
...
class AlertaAlerter(Alerter):
    ...
    def alert(self, matches):
        ...
        if (
            self.use_qk_as_resource
            and 'query_key' in self.rule
            and lookup_es_key(matches[0], self.rule['query_key'])
        ):
        ...
        try:
            response = requests.post(
                self.url, data=alerta_payload, headers=headers, verify=self.verify_ssl
            )
            response.raise_for_status()
        except RequestException as e:
    ...
    def get_info(self):
        return {'type': 'alerta', 'alerta_url': self.url}
    ...
    def get_json_payload(self, match):
        ...
        alerta_text = (
            self.rule['type'].get_match_str([match])
            if self.text == ''
            else resolve_string(self.text, match, self.missing_text)
        )
        alerta_event = (
            self.create_default_title([match])
            if self.event == ''
            else resolve_string(self.event, match, self.missing_text)
        )

        match_timestamp = lookup_es_key(
            match, self.rule.get('timestamp_field', '@timestamp')
        )
        ...
        alerta_payload_dict = {
            ...
            'service': [
                resolve_string(a_service, match, self.missing_text)
                for a_service in self.service
            ],
            'tags': [
                resolve_string(a_tag, match, self.missing_text) for a_tag in self.tags
            ],
            'correlate': [
                resolve_string(an_event, match, self.missing_text)
                for an_event in self.correlate
            ],
            'attributes': dict(
                list(
                    zip(
                        ...
                        [
                            resolve_string(a_value, match, self.missing_text)
                            for a_value in self.attributes_values
                        ],
                    )
                )
            ),
...
class HTTPPostJWTAlerter(Alerter):
    ...
    def __init__(self, rule):
        ...
        post_url = self.rule.get('http_post_url')
        self.post_jwt_key = self.rule.get('http_post_jwt_key')
        self.post_jwt_algorithm = self.rule.get('http_post_jwt_algorithm')
        if isinstance(post_url, str):
            post_url = [post_url]
        self.post_url = post_url
        self.post_proxy = self.rule.get('http_post_proxy')
    ...
    def alert(self, matches):
        ...
        for match in matches:
            ...
            for url in self.post_url:
                try:
                    """Encode payload using JWT"""
                    JWT_encode_payload = jwt.encode(
                        json.loads(json.dumps(payload, cls=DateTimeEncoder)),
                        self.post_jwt_key,
                        algorithm=self.post_jwt_algorithm,
                    )
                    response = requests.post(
                        url,
                        data=JWT_encode_payload,
                        headers=headers,
                        proxies=proxies,
                        timeout=self.timeout,
                    )
                    ...
                    raise EAException("Error posting HTTP JWT Post alert: %s" % e)
            elastalert_logger.info("HTTP JWT Post alert sent.")
                ...
                'content': [
                    {
                        ...
                        'content': [{'type': 'paragraph', 'content': parser.content}],
                    }
                ],
            }
        }
...
class LineNotifyAlerter(Alerter):
    ...
    def get_info(self):
        return {
            "type": "linenotify",
            "linenotify_access_token": self.linenotify_access_token,
        }
        ...
        for match in matches:
            ...
            for mapping in self.rule.get('hive_observable_data_mapping', []):
                for observable_type, match_data_key in mapping.items():
                    try:
                        match_data_keys = re.findall(
                            r'\{match\[([^\]]*)\]', match_data_key
                        )
                        rule_data_keys = re.findall(
                            r'\{rule\[([^\]]*)\]', match_data_key
                        )
                        data_keys = match_data_keys + rule_data_keys
                        context_keys = list(context['match'].keys()) + list(
                            context['rule'].keys()
                        )
                        if all(
                            [True if k in context_keys else False for k in data_keys]
                        ):
                            artifact = {
                                'tlp': 2,
                                'tags': [],
