...
class ElastAlerter():
    ...
    def get_query(filters, starttime=None, endtime=None, sort=True, timestamp_field='@timestamp', to_ts_func=dt_to_ts, desc=False):
        ...
        es_filters = {'bool': {'must': filters}}
        if starttime and endtime:
            starttime = to_ts_func(starttime)
            endtime = to_ts_func(endtime)
            es_filters['bool']['must'].insert(0, {'range': {timestamp_field: {'gt': starttime, 'lte': endtime}}})
        query = {'query': es_filters}
        if sort:
    ...
    def get_terms_query(self, query, size, field):
        ...
        if 'sort' in query:
            query.pop('sort')
        query.update({'aggs': {'elastalert': {'terms': {'field': field, 'size': size}}}})
        query.update({'size': 0 })
        return query
    ...
    def get_hits_count(self, rule, starttime, endtime, index):
        """ Query Elasticsearch for the count of results and returns a dictionary of timestamps
        ...
        try:
            res = self.current_es.search(index=index, doc_type=rule['doc_type'], body=query, ignore_unavailable=True)
        except ElasticsearchException as e:
            # Elasticsearch sometimes gives us GIGANTIC error messages
            # (so big that they will fill the entire terminal buffer)
            if len(str(e)) > 1024:
    ...
    def get_hits_terms(self, rule, starttime, endtime, index, key, qk=None, size=None):
        ...
        if 'aggregations' not in res:
            return {}
        buckets = res['aggregations']['elastalert']['buckets']
    ...
    def get_starttime(self, rule):
        ...
        """
        query = self.get_query([{'term': {'rule_name': '%s' % (rule['name'])}}], desc=True)
        try:
    ...
    def find_recent_pending_alerts(self, time_limit):
        ...
        query = self.get_query([{'query_string': {'query': '!_exists_:aggregate_id AND alert_sent:false'}}], starttime=(ts_now() - time_limit), endtime=ts_now(), timestamp_field="alert_time")
    ...
    def find_pending_aggregate_alert(self, rule, aggregation_key_value=None):
        query = get_query([
                          ...
                          timestamp_field="alert_time",
                          desc=True
                         )
        if aggregation_key_value:
            query['bool']['must'].append({'term': {'aggregate_key': aggregation_key_value}})
    ...
    def is_silenced(self, rule_name):
        ...
        query = self.get_query([{'term': {'rule_name': rule_name}}], timestamp_field="until", desc=True)
