...
class ElastAlerter():
    ...
    def get_aggregation_query(self, query, rule, query_key, terms_size, timestamp_field='@timestamp'):
        ...
        if not bucket_interval_period is None:
            if rule.get('bucket_interval_offset'):
                aggs_element = {'interval_aggs': {'date_histogram': {'field': timestamp_field, 'interval': bucket_interval_period , 'offset': '+%ss' % (rule['bucket_interval_offset'])},'aggs' : aggs_element}}
            else:
                aggs_element = {'interval_aggs': {'date_histogram': {'field': timestamp_field, 'interval': bucket_interval_period},'aggs' : aggs_element}}
        
        if not self.five:
            query_element['filtered'].update({'aggs': aggs })
            aggs_query = {'aggs': query_element}
        else:
            aggs_query = query
    ...
    def get_segment_size(self, rule):
        ...
        if not rule.get('use_count_query') and not rule.get('use_terms_query') and not rule.get('aggregation_query_element'):
            ...
            if rule.get('bucket_interval_delta'):
                return rule.get('bucket_interval_delta')
            else:
                return rule.get('buffer_time', self.buffer_time)
    ...
    def run_rule(self, rule, endtime, starttime=None):
        ...
        es_interval_delta = rule.get('bucket_interval_delta')  
        unix_starttime = dt_to_unix(rule['starttime'])
        if es_interval_delta and rule.get('aggregation_query_element'):
            es_interval_delta_in_sec = es_interval_delta.total_seconds()
            offset = int(unix_starttime % es_interval_delta_in_sec)   
            if rule.get('bucket_interval_sync'):
                rule['bucket_interval_offset'] = offset  
            else:
        ...
        if rule.get('aggregation_query_element') and (endtime - tmp_endtime) <  segment_size:
            endtime = tmp_endtime
        else:
