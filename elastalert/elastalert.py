...
class ElastAlerter():
    ...
    def get_aggregation_query(self, query, aggregation):
        """ Takes a query generated by get_query and outputs a aggregation query
        :param query: origin query
        :param aggregation: aggregation dictionary
        """
        if 'sort' in query:
            query.pop('sort')
    ...
    def get_aggregation_num(self, rule, starttime, endtime, index):
        """ Query elasticsearch for the given rule and return the results.
    ...
    def run_query(self, rule, start=None, end=None, scroll=False):
        ...
        if 'aggs' in rule and len(rule['aggs']) > 0:
            data = self.get_aggregation_num(rule, start, end, index)
